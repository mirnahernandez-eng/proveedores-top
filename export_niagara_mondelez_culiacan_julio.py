"""
export_niagara_mondelez_culiacan_julio.py
Hoja 1: NIAGARA  - BAE Culiacan  (CEDIS 7455) - Julio 2026
Hoja 2: MONDELEZ - SAM'S Culiacan (CEDIS 4971) - Julio 2026
Ordenado por SALIDA_HRS desc (tiempos mas altos primero)
"""
...
OUT_FILE = BASE / "niagara_mondelez_culiacan_julio.xlsx"

SHEETS = [
    {"titulo": "NIAGARA BAE CULIACAN",   "cedis": 7455, "vendor_like": "%NIAGARA%",  "hdr": "1F497D", "alt": "DCE6F1", "tot": "9DC3E6"},
    {"titulo": "MONDELEZ SAMS CULIACAN", "cedis": 4971, "vendor_like": "%MONDELEZ%", "hdr": "375623", "alt": "E2EFDA", "tot": "A9D18C"},
]

# ─── Query ────────────────────────────────────────────────────────────────────
def query_vendor(cedis: int, vendor_like: str) -> pd.DataFrame:
    q = f"""
    SELECT
    ...
    WHERE ARRIVAL_DATE BETWEEN '2026-07-01' AND '2026-07-31'
      ...
      AND UPPER(VENDOR) LIKE '{vendor_like}'
...
COL_W = {
    ...
    "PAPER_W":10,"SALIDA_HRS":14,"SERVICIO_HRS":14,
...
# ─── Write sheet ──────────────────────────────────────────────────────────────
def write_sheet(ws, df: pd.DataFrame, cfg: dict):
    ws.title = cfg["titulo"][:31]
    ncols    = len(COLS)
    hdr_fill = PatternFill("solid", fgColor=cfg["hdr"])
    alt_fill = PatternFill("solid", fgColor=cfg["alt"])
    tot_fill = PatternFill("solid", fgColor="2E4057")

    # Calcular umbrales
    sal_vals = df["SALIDA_HRS"].dropna()
    sal_vals = sal_vals[sal_vals > 0]
    avg_sal  = sal_vals.mean() if len(sal_vals) else 0
    p75_sal  = sal_vals.quantile(0.75) if len(sal_vals) else 0
    ...
    t = ws.cell(1, 1,
        f"{cfg['titulo']} | Julio 2026 | {len(df):,} citas | "
        f"Prom salida: {fmt_hrs(avg_sal)} | P75: {fmt_hrs(p75_sal)} | ordenado desc")
    t.fill = hdr_fill
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ri       = i + 3
        sal_v    = float(row.get("SALIDA_HRS") or 0)
        row_fill = red_fill if sal_v >= p75_sal and sal_v > 0 else (org_fill if sal_v >= avg_sal and sal_v > 0 else (alt_fill if i % 2 == 0 else None))
        ...
        for j, col in enumerate(COLS, 1):
            ...
            if col == "PUNTUALIDAD":
                ...
                dc.fill = red_fill if sal_v >= p75_sal and sal_v > 0 else (org_fill if sal_v > 0 else (row_fill or PatternFill()))
                dc.font = Font(bold=True, size=9,
                               color="C00000" if sal_v >= p75_sal and sal_v > 0 else ("7F6000" if sal_v > 0 else "000000"))
            else:
    ...
    for txt, fill, desc in [
        ("ROJO:",    red_fill, f"Salida >= P75 ({fmt_hrs(p75_sal)})"),
        ("AMARILLO:",org_fill, f"Salida >= Promedio ({fmt_hrs(avg_sal)})"),
    ]:
...
for cfg in SHEETS:
    print(f"Consultando {cfg['titulo']} (CEDIS {cfg['cedis']}) ...")
    df = query_vendor(cfg["cedis"], cfg["vendor_like"])
    print(f"  {len(df):,} citas")
    ws = wb.active if first else wb.create_sheet()
    first = False
    write_sheet(ws, df, cfg)
