...
function renderAll() {
  ...
  var swOver = useSW ? [activePeriod] : null;
...
function buildTable(title, data, objData, order, regions, loc, period, hasObj, overridePeriods, filtLocs) {
  ...
  // ── Periodos visibles: mes anterior + mes en curso + YTD ─────────────────
  // Solo se aplica en modo mes (no SW). En modo SW se muestran todos.
  var visPer;
  if (!overridePeriods) {
    ...
    visPer = [];
    if (lastMIdx > 0) visPer.push(ALL_MONTHS_FULL[lastMIdx - 1]); // mes anterior
    if (lastMIdx >= 0) visPer.push(ALL_MONTHS_FULL[lastMIdx]);     // mes en curso
    visPer.push('ytd');                                              // YTD siempre
    // Si el usuario seleccionó un mes anterior que no está, añadirlo antes de YTD
    if (period && period !== 'ytd' && visPer.indexOf(period) < 0) {
      visPer.splice(visPer.length - 1, 0, period);
    }
  } else {
    visPer = PERIODS; // modo SW: mostrar todos los periodos recibidos
  }
  var visActIdx = visPer.indexOf(period);

  if (isAll) {
    // ── VISTA NACIONAL: Objetivo 2026 + meses visibles (sin desglose por CD) ──
    var trH = document.createElement("tr");
    trH.appendChild(th("Proveedores TOP", 1, 1, "v-name text-left", "#1e3a8a"));
    trH.appendChild(th("Objetivo", 1, 1, "", "#1e3a8a"));
    for (var vi2 = 0; vi2 < visPer.length; vi2++) {
      var bg = PER_BG[visPer[vi2]] || "#1d4ed8";
      trH.appendChild(th(PER_LBL[visPer[vi2]] || visPer[vi2], 1, 1, "", bg));
    }
    ...
    for (var vi = 0; vi < order.length; vi++) {
      ...
      var anyData = false;
      for (var vi2 = 0; vi2 < visPer.length; vi2++) {
        if (getVal(data, v, visPer[vi2], loc, null) != null) { anyData = true; break; }
      }
      if (!anyData) continue;
      ...
      for (var vi2 = 0; vi2 < visPer.length; vi2++) {
        var val    = getVal(data, v, visPer[vi2], loc, null);
        var actCls = (vi2 === visActIdx) ? " col-active" : "";
        var cls    = (hasObj ? cellCls(val, obj26) : (val===null?"cell-null":"")) + actCls;
    ...
    for (var vi2 = 0; vi2 < visPer.length; vi2++) {
      var val    = getVal(data, "__total__", visPer[vi2], loc, null);
      var actCls = (vi2 === visActIdx) ? " col-active" : "";
      var tcls   = (hasObj ? cellCls(val, objTot26) : (val===null?"cell-null":"")) + " font-bold" + actCls;
      totRow.appendChild(td(fmt(val), tcls.trim()));
    }
    tbody.appendChild(totRow);

  } else {
    // ── VISTA SIMPLE: proveedor | objetivo | periodos visibles ──
    var trH = document.createElement("tr");
    trH.appendChild(th("Proveedores TOP", 1, 1, "v-name text-left", "#1e3a8a"));
    trH.appendChild(th("Objetivo", 1, 1, "", "#1e3a8a"));
    for (var vi2 = 0; vi2 < visPer.length; vi2++) {
    ...
    for (var vi = 0; vi < order.length; vi++) {
      ...
      var anyData = false;
      for (var vi2 = 0; vi2 < visPer.length; vi2++) {
        if (getVal(data, v, visPer[vi2], loc, filtLocs) != null) { anyData = true; break; }
      }
      if (!anyData) continue;
      var row = document.createElement("tr");
      var tdN = td(v, "v-name"); tdN.title = v; row.appendChild(tdN);
      ...
      for (var vi2 = 0; vi2 < visPer.length; vi2++) {
        var val    = getVal(data, v, visPer[vi2], loc, filtLocs);
        var actCls = (vi2 === visActIdx) ? " col-active" : "";
        var cls    = (hasObj ? cellCls(val, objL) : (val === null ? "cell-null" : "")) + actCls;
        row.appendChild(td(fmt(val), cls.trim()));
      }
      tbody.appendChild(row);
    }
    ...
    for (var vi2 = 0; vi2 < visPer.length; vi2++) {
      var val    = getVal(data, "__total__", visPer[vi2], loc, filtLocs);
      var actCls = (vi2 === visActIdx) ? " col-active" : "";
      var clsTot = (hasObj ? cellCls(val, objLT) : (val===null?"cell-null":"")) + " font-bold" + actCls;
